...
#include "EquestPerspectiveWidget.h"
...
//		   ccfg.motionDataConfiguration.userConfiguration.password = "testUser";
   ...
   QToolButton* exporterButton = controller.createButton(tr("Exporter"), QIcon(":/equest/icons/Operacje.png"));
   controller.addToolbarButton(exporterButton);
   connect(exporterButton, SIGNAL(clicked()), this, SLOT(showEquestExporterDialog()));
...
void MdeMainWindow::showEquestExporterDialog()
{
	auto exporter = core::queryService<equestExporter::IEquestExporterService>(plugin::getServiceManager());
	if (exporter) {
