...
//void AdnotationsDelegate::setImage(equest::ILayeredImageConstPtr val)
//{
//	this->val = val;
//}
				...
				if(aidx > 7){
					...
					if(column == 1){
						...
						case annotations::bloodLevel:
							{
								auto adn = equest::annotations::instanceBloodLevels();
						...
						case annotations::inflammatoryLevel:
							{
								auto adn = equest::annotations::instanceInflammatoryLevels();
								for(auto it = adn->left.begin(); it != adn->left.end(); ++it){
									editor->addItem(it->second, it->first);
								}
							}
						...
						case annotations::fingerType:
							{
								auto adn = equest::annotations::instanceFingerTypes();
								for(auto it = adn->left.begin(); it != adn->left.end(); ++it){
									editor->addItem(it->second, it->first);
								}
							}
						...
						case annotations::jointType:
							{
								auto adn = equest::annotations::instanceJointTypes();
								for(auto it = adn->left.begin(); it != adn->left.end(); ++it){
									editor->addItem(it->second, it->first);
								}
							}
						...
						case annotations::imageType:
							{
								auto adn = equest::annotations::instanceImageTypes();
								for(auto it = adn->left.begin(); it != adn->left.end(); ++it){
									editor->addItem(it->second, it->first);
								}
							}
