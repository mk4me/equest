#include <EquestPCH.h>
#include <plugins/equest/AnnotationStatusFilter.h>
...
{
	try{
		bool userIsReviewer = equest::EquestService::userIsReviewer(session);
...
void AnnotationStatusFilter::refreshIdentifiers()
{
	...
	switch (status)
	{
	case equest::AnnotationStatusFilter::InEdition:
		identifiers = inEdition;
		break;
	case equest::AnnotationStatusFilter::InVerification:
		identifiers = inVerification;
		break;
	case equest::AnnotationStatusFilter::Verified:
